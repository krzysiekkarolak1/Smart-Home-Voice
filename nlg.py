...
class NLG(object):
    ...
    def acknowledge(self):
        ...
        personal_acknowledgement = [
            "W czym mogę Ci dzisiaj pomóc, %s?" % user_name,
            "W czym mogę Ci pomóc, %s?" % user_name,
            "Co mogę dla Ciebie zrobić, %s?" % user_name,
            "Witaj %s, co mogę dla Ciebie zrobić?" % user_name,
            "No elo %s, co mogę dla Ciebie zrobić?" % user_name
        ]
        ...
        if choice == 0:
            ...
            ret_phrase = "Good %s. Co mogę dla Ciebie zrobić?" % self.time_of_day(date)
        else:
    ...
    def user_status(self, type='positive', attribute=None):
        ...
        if attribute is None:
            ret_phrase = "Wyglądasz %s" % random.choice(complement_choice)
        else:
            ret_phrase = self.generate('none', {'subject': "Twoje %s" % attribute, 'verb': 'wyglądają %s' % random.choice(complement_choice)}, "present")
    ...
    def news(self, tense):
        ...
        if choice == 1:
            ret_phrase = self.generate('none', {'subject': "Znalazłam ", 'object': random.choice(headlines_nouns), 'verb': 'find', 'objmodifiers': random.choice(headlines_adjectives), 'preposition': 'dla', 'prepmodifiers': [random.choice(headlines_prepmodifiers)]}, tense)
        else:
            ret_phrase = self.generate('none', {'subject': "", 'object': random.choice(headlines_nouns), 'verb': 'find', 'objmodifiers': random.choice(headlines_adjectives)}, tense)
    ...
    def article_interest(self, article_titles):
        ...
        if random.randint(0,2) == 0: # 1 in 3 chance the bot will express interest in a random article
            if article_titles is not None:
                ...
                ret_phrase = "%s to jeden z nich" % article
    ...
    def greet(self):
        ...
        if (choice == 0) or (choice == 3): # time related
            ret_phrase = "Dobry %s" % self.time_of_day(datetime.datetime.now())
            if self.user_name is not None:
