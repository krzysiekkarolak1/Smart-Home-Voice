...
weather_api_token = "108a43245cdbcea3fdcbb08973e02581"
wit_ai_token = "Bearer 6EJRDTR3ZO7UFWEOIXKV2HQWIPVYHDKW"
...
class Bot(object):
        ...
        if speech is not None:
            try:
                ...
                if intent == 'greeting':
                    ...
                    self.__text_action("Przepraszam, jeszcze tego nie wiem.")
                    return
                ...
                print(e)
                traceback.print_exc()
                self.__text_action("Przepraszam, nie wiem co miałeś na myśli...")
                return
    ...
    def __joke_action(self):
        ...
        if joke is not None:
            ...
            self.__text_action("Nie mogę znaleźć żadnych żartów")
    ...
    def __stories_action(self):
        headliness = self.knowledge.get_stories()

        if headliness:
            requests.post("http://localhost:8080/news", data=json.dumps({"articles":headliness}))
            self.speech.synthesize_text(self.nlg.news("past"))
            interest = self.nlg.article_interest(headliness)
            if interest is not None:
                self.speech.synthesize_text(interest)
        else:
            self.__text_action("I had some trouble finding wykop for you")
    ...
    def __news_action(self):
        ...
        if headlines:
            ...
            self.__text_action("Miałam kilka problemów w szukaniu wiadomości")
    
    def __light_action(self):
            dd = self.knowledge.get_light()
            self.__text_action("Jasne! Zrobione.")
    ...
    def __maps_action(self, nlu_entities=None):
        ...
        if location is not None:
            ...
            maps_action = "Oczywiście. Tu jest mapa dla lokalizacji %s." % location
            body = {'url': maps_url}
            requests.post("http://localhost:8080/image", data=json.dumps(body))
            self.speech.synthesize_text(maps_action)
        else:
            self.__text_action("Przepraszam, nie zrozumiałam danych lokalizacji. Powtórz proszę.")
